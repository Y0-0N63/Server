...
jsp : html, css, js, java, el, jstl 작성 가능 + 동적 페이지 (요청에 따라 응답 화면이 변함)
	</pre>
	...
	<h1>
		<%-- a 태그 클릭시 /el/scope 주소로 GET 방식 요청 --%>
	...
	<pre>
- 클라이언트 요청을 받은 Servlet/JSP가 직접 응답하는것이 아닌 다른 Servlet/JSP에 HttpServletRequest, HttpServletResponse 객체를 넘겨서(위임) 대신 응답하게 하는 것
...
- 응답 화면(결과 페이지) 주소는 처음 Servlet이 요청 받은 주소 그대로!
	...
	<pre>
- 클라이언트의 요청을 받은 Servlet에서 직접 응답하지 않고, 다른 Servlet을 다시 요청하는 것
- 보통 Servlet 요청 처리 후 특정 JSP로 요청을 위임해 결과 화면을 응답해 주는 것이 아닌 (요청 받은 Servlet이 응답해줄 JSP가 없음)
	다른 Servlet을 다시 요청하여 다른 Servlet의 결과 화면을 응답해줌
- redirect는 다시 요청 하는 것!!! -> 기존 req, resp 객체가 사라지고 새로운 req, resp 객체가 생성된다.
