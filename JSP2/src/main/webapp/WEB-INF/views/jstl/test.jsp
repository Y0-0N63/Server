...
<hr>
	...
	<pre>
- scope 객체에 세팅된 속성을 제거하는 태그 (= 객체.removeAttribute("key"))
		
[속성]
1) var : 변수명 (== key)
2) scope : 범위 지정(page, request, session, application) -> 기본값 : 전 범위 일괄 제거
	</pre>
...
<hr>
	...
	<pre>
[c:choose 태그]
- 내부에 조건문 태그를 작성하겠다는 태그 -> 내부에 c:when, c:otherwise 태그만 작성 가능!!
		
* 주의 사항 *
바로 한단계 아래에는 c:when, c:otherwise, 공백, jsp용 주석(<%-- --%>)만 작성할 수 있고 나머지 모두 에러 발생!!
		
[c:when 태그]
- if(조건식) / else if(조건식) 를 나타내는 태그 -> test 속성에 조건식 작성
...
- else를 나타내는 태그 -> 아무 속성도 없음!
	...
	<c:choose>
		<%-- empty 연산자 : E에서 사용하는 연산자
			- null인 경우 > true
			- 연산 대상이 배열이거나 컬렉션인데, 내부에 아무 요소도 없는 경우 > true
			- 나머지 > false --%>
...
6) varStatus : 현재 반복 상태와 관련된 정보를 제공하는 변수 선언
  ...
  * varStatus에서 제공되는 값
	...
	<li>
		<%-- 접두사 fn이 추가된 EL 구문 사용에서 fu(function, 기능) : 문자열 컬렉션 관련 제공하는 JSTL = EL	--%>
