...
public class JDBCTemplate {
	...
	public static Connection getConnection() {
		try {
			// 이전에 참조하던 Connection 객체가 존재하고 아직 close() 된 상태가 아니라면 새로 만들지 않고 기존 Connection 반환
			...
			// 현재클래스명.class.getResource("경로") : * 클래스패스(classpath) 내에서 지정된 리소스 파일을 찾는 메서드
			// * classpath란?
			// Java 프로그램이 클래스를 찾기 위해 검색하는 경로를 의미 -> src/main/resources 또는 WEB-INF/classes 에서 찾음
